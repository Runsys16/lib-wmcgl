...
GLhandleARB Shader::linkShaders(GLhandleARB* object, const unsigned int& nb)
{
  ...
  for(unsigned int i = 0; i < nb; ++i){
	...
	if ( nb == 3 )
	{
		glProgramParameteriEXT(po, GL_GEOMETRY_INPUT_TYPE_EXT, GL_TRIANGLES);
		glProgramParameteriEXT(po, GL_GEOMETRY_OUTPUT_TYPE_EXT, GL_TRIANGLES);
		
		glProgramParameteriEXT(po, GL_GEOMETRY_VERTICES_OUT_EXT, max);	
		glGetIntegerv(GL_MAX_GEOMETRY_OUTPUT_VERTICES, &max);
		//std::cout << "GL_MAX_GEOMETRY_OUTPUT_VERTICES " << max << std::endl;
		glGetIntegerv(GL_MAX_GEOMETRY_TOTAL_OUTPUT_COMPONENTS, &max);
		//std::cout << "GL_MAX_GEOMETRY_TOTAL_OUTPUT_COMPONENTS " << max << std::endl;
	}
...
bool Shader::Load(const std::string& name, const std::string& extensions)
{
	...
	std::string s2 = "./shaders/"+name+".geo";
	so[1] = loadShader(s2);
	if(so[1]==0){
		//std::cerr << "Pas de  shader "+s2+" failed (exiting...)" << std::endl;

			std::string s3 = "./shaders/"+name+".frag";
			so[1] = loadShader(s3);
			if(so[1]==0){
				std::cerr << "loading shader "+s3+" failed (exiting...)" << std::endl;
				return false;
			}
			if(!compileShader(so[1])){
				std::cerr << "compiling shader "+s3+" failed (exiting...)" << std::endl;
				return false;
			}
			...
			if ( m_nProgram == 0 )	std::cerr << "link error shader" << name << std::endl;
			else					std::cerr << "link shader ok";// << std::endl;
	...
	if(!compileShader(so[2])){
		std::cerr << "compiling shader "+s3+" failed (exiting...)" << std::endl;
		return false;
	}

	
	m_nProgram = linkShaders(so,3);
	if ( m_nProgram == 0 )	std::cerr << "link error shader" << name << std::endl;
	else					std::cerr << "link shader ok";// << std::endl;
	glDeleteObjectARB(so[0]);
	glDeleteObjectARB(so[1]);
	glDeleteObjectARB(so[2]);
